...
- `distributed_cache_management_enabled` (Boolean, Deprecated) Enable or disable managing of cache instances over the message bus. The default value is `true`. Deprecated since SEMP API version 2.28. Distributed cache management is now redundancy aware and thus no longer requires administrative intervention for operational state.
